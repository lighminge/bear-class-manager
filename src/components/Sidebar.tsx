    ...
    <>
      ...
      <div className="fixed left-0 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-40">
        <button
          ...
          className={`flex flex-col items-center justify-center p-2 py-4 gap-3 rounded-r-xl border-y-2 border-r-2 shadow-xl transition-colors ${
            activeTab === 'attendance' ? 'bg-yellow-600 border-yellow-400' : 'bg-black/50 border-white/20 hover:bg-black/70'
          }`}
          title="點名簿"
        >
          <Users className="w-6 h-6 text-white" />
          <span className="font-bold text-white tracking-[0.2em] hidden sm:inline" style={{ writingMode: 'vertical-rl', textOrientation: 'upright' }}>點名簿</span>
        </button>
        <button
        ...
        >
          ...
          <span className="font-bold text-white tracking-[0.2em] hidden sm:inline" style={{ writingMode: 'vertical-rl', textOrientation: 'upright' }}>老師請假</span>
        </button>
      </div>
